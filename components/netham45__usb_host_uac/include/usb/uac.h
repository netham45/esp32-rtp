...
typedef enum {
    ...
    UAC_AC_CLOCK_SOURCE                               = 0x0A,
    UAC_AC_CLOCK_SELECTOR                             = 0x0B,
    UAC_AC_CLOCK_MULTIPLIER                           = 0x0C,
    UAC_AC_SAMPLE_RATE_CONVERTER                      = 0x0D
...
/**
 * @brief Audio 2.0 class-specific request codes
 */
typedef enum {
    UAC2_CS_REQ_UNDEFINED                             = 0x00,
    UAC2_CS_REQ_CUR                                   = 0x01,
    UAC2_CS_REQ_RANGE                                 = 0x02,
    UAC2_CS_REQ_MEM                                   = 0x03,
} uac2_cs_request_code_t;

/**
 * @brief UAC 2.0 Clock Source control selectors
 */
typedef enum {
    UAC2_CLOCK_CTRL_UNDEFINED                         = 0x00,
    UAC2_CLOCK_CTRL_SAM_FREQ                          = 0x01,
    UAC2_CLOCK_CTRL_CLOCK_VALID                       = 0x02,
} uac2_clock_control_selector_t;

/**
 * @brief UAC 2.0 Clock Selector control selectors
 */
typedef enum {
    UAC2_CLOCK_SELECTOR_CONTROL                       = 0x01,
} uac2_clock_selector_control_t;
...
/******************************** UAC 2.0 Control Helpers *********************************/

/**
 * @brief Control access capabilities for UAC 2.0 entities
 */
typedef enum {
    UAC_CTRL_ACCESS_NONE                               = 0x00,
    UAC_CTRL_ACCESS_R                                  = 0x01,
    UAC_CTRL_ACCESS_RW                                 = 0x03,
} uac_ctrl_access_t;

/**
 * @brief UAC 2.0 Clock Source Control positions (lsb bit offsets)
 */
typedef enum {
    UAC_CLOCK_SOURCE_CTRL_SAM_FREQ_POS                 = 0,
    UAC_CLOCK_SOURCE_CTRL_CLOCK_VALID_POS              = 2,
} uac_clock_source_ctrl_pos_t;

/**
 * @brief UAC 2.0 Feature Unit control bit positions (lsb bit offsets)
 */
typedef enum {
    UAC2_FU_CTRL_MUTE_POS                              = 0,
    UAC2_FU_CTRL_VOLUME_POS                            = 2,
    UAC2_FU_CTRL_BASS_POS                              = 4,
    UAC2_FU_CTRL_MID_POS                               = 6,
    UAC2_FU_CTRL_TREBLE_POS                            = 8,
    UAC2_FU_CTRL_GRAPHIC_EQ_POS                        = 10,
    UAC2_FU_CTRL_AUTOMATIC_GAIN_POS                    = 12,
    UAC2_FU_CTRL_DELAY_POS                             = 14,
    UAC2_FU_CTRL_BASS_BOOST_POS                        = 16,
    UAC2_FU_CTRL_LOUDNESS_POS                          = 18,
} uac2_fu_control_pos_t;
...
typedef struct {
    ...
    uint8_t bClockID;
    uint8_t bNrInPins;
    uint8_t baCSourceID[];    /*!< List of upstream clock IDs followed by bmControls + iClockSource */
...
typedef struct {
    ...
    uint16_t bmControls;
    uint8_t iTerminal;
} __attribute__((packed)) uac2_ac_input_terminal_desc_t;
...
typedef struct {
    ...
    uint16_t bmControls;
    uint8_t iTerminal;
} __attribute__((packed)) uac2_ac_output_terminal_desc_t;
...
typedef struct {
    ...
    uint8_t bUnitID;
    uint8_t bSourceID;
    uint32_t bmaControls[];   /*!< Controls for master + logical channels */
} __attribute__((packed)) uac2_ac_feature_unit_desc_t;
...
typedef struct {
    ...
    uint8_t bBitResolution;
} __attribute__((packed)) uac2_as_type_I_format_desc_t;
